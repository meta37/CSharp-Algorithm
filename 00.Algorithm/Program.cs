namespace _00.Algorithm
{
    internal class Program
    {
         ...
         * 알고리즘 성능
         * 
         * 효율적인 문제해결을 위해선 알고리즘의 성능을 판단할 수 있는 기준이 필요 -> 어떨때 뭐가 빠른지
         * 상황에 따라 적합한 알고리즘을 선택할 수 있도록 하는 기준
         ***************************************************************************/

        // <알고리즘 평가 기준>
        // 컴퓨터에서 알고리즘과 자료구조의 평가는 시간과 공간 두 자원을 얼마나 소모하는지가 효율성의 중점 ex) 둘다 1초 걸리는데 1mb vs 1gb
        // 일반적으로 시간을 위해 공간이 희생되는 경우가 많음
        // 시간복잡도 : 알고리즘의 시간적 자원 소모량
        // 공간복잡도 : 알고리즘의 공간적 자원 소모량

        // A 알고리즘 쓰면 100초 걸린다. (고철컴퓨터로)
        // B 알고리즘 쓰면 1초 걸린다. (슈퍼컴퓨터로)  --> 컴퓨터 성능마다 상황이 다르므로 단순 시간으로 판단할 수 없음


        // <Big-O 표기법>
        // 알고리즘의 복잡도를 나타내는 점근표기법  => 대략적으로 표현함
        // 데이터 증가량에 따른 시간 증가량을 계산
        // 가장 높은 차수의 계수와 나머지 모든 항을 제거하고 표기 ex) (n²+2n+1)  n²을 제외하고 다 제거 -> (n²)
        ...
        // Big-O		 O(1)	     O(n)   	   O(n²)
        // 효율적인 순서 Case1 > Case2 > Case3


        // <수행 시간 분석>
        // 알고리즘의 성능은 상황에 따라 수행 시간이 달라짐
        // 수행 시간을 분석하는 경우 평균의 상황과 최악의 상황을 기준으로 평가함
        // 최선의 상황의 경우 알고리즘의 성능을 분석하는 수단으로 적합하지 않음 -> 운 좋으면 바로 찾을 수 있어서
